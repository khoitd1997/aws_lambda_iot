...
class _shadowRequestToken:
    ...
    def getNextToken(self):
        # We only need the uuid digits, not the urn prefix
        return uuid.uuid4().urn[self.URN_PREFIX_LENGTH:]
...
class deviceShadow:
    ...
    def _doNonPersistentUnsubscribe(self, currentAction):
        self._shadowManagerHandler.basicShadowUnsubscribe(
            self._shadowName, currentAction)
        self._logger.info("Unsubscribed to " + currentAction +
                          " accepted/rejected topics for deviceShadow: " + self._shadowName)
    ...
    def generalCallback(self, client, userdata, message):
        ...
        with self._dataStructureLock:
            ...
            currentAction = self._parseTopicAction(
                currentTopic)  # get/delete/update/delta
            currentType = self._parseTopicType(
                currentTopic)  # accepted/rejected/delta
            ...
            if currentAction in ["get", "delete", "update"]:
                ...
                if self._basicJSONParserHandler.validateJSON():  # Filter out invalid JSON
                    currentToken = self._basicJSONParserHandler.getAttributeValue(
                        u"clientToken")
                    if currentToken is not None:
                        self._logger.debug(
                            "shadow message clientToken: " + currentToken)
                    # Filter out JSON without the desired token
                    if currentToken is not None and currentToken in self._tokenPool.keys():
                        # Sync local version when it is an accepted response
                        self._logger.debug(
                            "Token is in the pool. Type: " + currentType)
                        if currentType == "accepted":
                            incomingVersion = self._basicJSONParserHandler.getAttributeValue(
                                u"version")
                            ...
                            # If it is a delete accepted, we need to reset the version
                                ...
                                # The version will always be synced for the next incoming delta/GU-accepted response
                                self._lastVersionInSync = -1
                        ...
                        if not self._isPersistentSubscribe and self._shadowSubscribeStatusTable.get(currentAction) <= 0:
                            ...
                            processNonPersistentUnsubscribe = Thread(
                                target=self._doNonPersistentUnsubscribe, args=[currentAction])
                            processNonPersistentUnsubscribe.start()
                        # Custom callback
                        if self._shadowSubscribeCallbackTable.get(currentAction) is not None:
                            processCustomCallback = Thread(target=self._shadowSubscribeCallbackTable[currentAction], args=[
                                                           payloadUTF8String, currentType, currentToken])
            ...
            # delta: Watch for version
                ...
                if self._basicJSONParserHandler.validateJSON():  # Filter out JSON without version
                    ...
                    if incomingVersion is not None and incomingVersion > self._lastVersionInSync:
                        ...
                        if self._shadowSubscribeCallbackTable.get(currentAction) is not None:
                            processCustomCallback = Thread(target=self._shadowSubscribeCallbackTable[currentAction], args=[
                                                           payloadUTF8String, currentType, None])
                            processCustomCallback.start()
    ...
    def _timerHandler(self, srcActionName, srcToken):
        with self._dataStructureLock:
            ...
            if srcToken not in self._tokenPool:
                self._logger.warn(
                    'Tried to remove non-existent token from pool: %s' % str(srcToken))
                return
            # Remove the token
            ...
            if not self._isPersistentSubscribe and self._shadowSubscribeStatusTable.get(srcActionName) <= 0:
                ...
                self._shadowManagerHandler.basicShadowUnsubscribe(
                    self._shadowName, srcActionName)
            # Notify time-out issue
            if self._shadowSubscribeCallbackTable.get(srcActionName) is not None:
                self._logger.info("Shadow request with token: " +
                                  str(srcToken) + " has timed out.")
                self._shadowSubscribeCallbackTable[srcActionName](
                    "REQUEST TIME OUT", "timeout", srcToken)
    ...
    def shadowGet(self, srcCallback, srcTimeout):
        ...
        **Returns**
        ...
        with self._dataStructureLock:
            ...
            self._tokenPool[currentToken] = Timer(
                srcTimeout, self._timerHandler, ["get", currentToken])
            self._basicJSONParserHandler.setString("{}")
            self._basicJSONParserHandler.validateJSON()
            self._basicJSONParserHandler.setAttributeValue(
        ...
        if not self._isPersistentSubscribe or not self._isGetSubscribed:
            self._shadowManagerHandler.basicShadowSubscribe(
                self._shadowName, "get", self.generalCallback)
            self._isGetSubscribed = True
            self._logger.info(
                "Subscribed to get accepted/rejected topics for deviceShadow: " + self._shadowName)
        # One publish
        self._shadowManagerHandler.basicShadowPublish(
            self._shadowName, "get", currentPayload)
        # Start the timer
        self._logger.info("Starting shadow get timer")
    ...
    def shadowDelete(self, srcCallback, srcTimeout):
        ...
        **Returns**
        ...
        with self._dataStructureLock:
            ...
            currentToken = self._tokenHandler.getNextToken()
            self._tokenPool[currentToken] = Timer(
                srcTimeout, self._timerHandler, ["delete", currentToken])
            self._basicJSONParserHandler.setString("{}")
            self._basicJSONParserHandler.validateJSON()
            self._basicJSONParserHandler.setAttributeValue(
                "clientToken", currentToken)
        ...
        if not self._isPersistentSubscribe or not self._isDeleteSubscribed:
            self._shadowManagerHandler.basicShadowSubscribe(
                self._shadowName, "delete", self.generalCallback)
            self._isDeleteSubscribed = True
            self._logger.info(
                "Subscribed to delete accepted/rejected topics for deviceShadow: " + self._shadowName)
        # One publish
        self._shadowManagerHandler.basicShadowPublish(
            self._shadowName, "delete", currentPayload)
        # Start the timer
        self._tokenPool[currentToken].start()
        return currentToken
        ...
        **Returns**
        ...
        if self._basicJSONParserHandler.validateJSON():
            with self._dataStructureLock:
                ...
                currentToken = self._tokenHandler.getNextToken()
                self._tokenPool[currentToken] = Timer(
                    srcTimeout, self._timerHandler, ["update", currentToken])
                self._basicJSONParserHandler.setAttributeValue(
                    "clientToken", currentToken)
            ...
            if not self._isPersistentSubscribe or not self._isUpdateSubscribed:
                self._shadowManagerHandler.basicShadowSubscribe(
                    self._shadowName, "update", self.generalCallback)
                self._isUpdateSubscribed = True
                self._logger.info(
                    "Subscribed to update accepted/rejected topics for deviceShadow: " + self._shadowName)
            # One publish
            self._shadowManagerHandler.basicShadowPublish(
                self._shadowName, "update", JSONPayloadWithToken)
            # Start the timer
            self._tokenPool[currentToken].start()
        else:
    ...
    def shadowRegisterDeltaCallback(self, srcCallback):
        ...
        **Returns**
        ...
        # One subscription
        self._shadowManagerHandler.basicShadowSubscribe(
            self._shadowName, "delta", self.generalCallback)
        self._logger.info(
            "Subscribed to delta topic for deviceShadow: " + self._shadowName)
    ...
    def shadowUnregisterDeltaCallback(self):
        ...
        **Returns**
        ...
        # One unsubscription
        self._shadowManagerHandler.basicShadowUnsubscribe(
            self._shadowName, "delta")
        self._logger.info(
            "Unsubscribed to delta topics for deviceShadow: " + self._shadowName)
